...
export default function AnimatedTitle() {
  ...
  const react = useAnimationControls();
  const nextJs = useAnimationControls();
  ...
  useEffect(() => {
    ...
    const animationActions = [
      ...
      { controller: web, value: 'hidden' },
      { controller: react as AnimationControls, value: 'visible' }, 
      { controller: backEnd, value: 700 }, 
      { controller: react, value: 'hidden' },  
  ...
  return (
    <div className="flex text-blue-100">
      ...
      <motion.div variants={list} initial="hidden" animate={frontEnd}>
        <WriteWord word="Full Stack Developer" />
      </motion.div>
      <div className="text-transparent">a</div>
      <motion.div variants={list} initial="hidden" animate={backEnd}>
        <WriteWord word="Frontend Developer" />
      </motion.div>
      <motion.div variants={list} initial="hidden" animate={react}>
        <WriteWord word="React Developer" />
      </motion.div>
    </div>
  );
}
